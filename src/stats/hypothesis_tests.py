...
def hypothesis_tests(df: pd.DataFrame, config_path: str):
    ...
    claim_freq = [df[df['Province'] == p]['HasClaim'].mean() for p in provinces]
    chi2, pval = stats.chi2_contingency(pd.crosstab(df['Province'], df['HasClaim']))[:2]
    results.append({
    ...
    zip_codes = df['PostalCode'].value_counts().head(10).index  # Top 10 for simplicity
    logger.info(f"Testing zip codes: {zip_codes}")
    claim_freq = [df[df['PostalCode'] == z]['HasClaim'].mean() for z in zip_codes]
    chi2, pval = stats.chi2_contingency(pd.crosstab(df[df['PostalCode'].isin(zip_codes)]['PostalCode'], df['HasClaim']))[:2]
    results.append({
    ...
    claim_freq = [df[df['Gender'] == g]['HasClaim'].mean() for g in genders]
    chi2, pval = stats.chi2_contingency(pd.crosstab(df[df['Gender'].isin(genders)]['Gender'], df['HasClaim']))[:2]
    results.append({
    ...
    logger.info("Hypothesis test results saved to reports/hypothesis_test_results.csv")
